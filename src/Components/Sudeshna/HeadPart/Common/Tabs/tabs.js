...
const Tabs = ({actTab, setActTab }) => {
  return (
    <div className="tab-options">
      <div className="max-width options-wrapper">
        {difftabs.map((tab) => {
          return (
            <div onClick={() => setActTab(tab.name)} 
             className={`tab-item absolute-center cur-po ${actTab===tab.name && "active-tab"}`}>

              <div className='tab-image-container absolute-center' 
              style={{
                backgroundColor:`${
                  actTab===tab.name ? tab.backdrop : ""
                }`,
              }}
              > 
                ...
                <img 
                 ...
                 alt={tab.name} 
                 src={ 
                   actTab === tab.name ? tab.active_img : tab.inactive_img
                  }
                 />
                </div> 
